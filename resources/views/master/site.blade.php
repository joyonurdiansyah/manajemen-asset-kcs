...
@section('content')
    ...
    <div class="modal fade" id="deleteSiteModal" tabindex="-1" aria-labelledby="deleteSiteModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="mb-3 col-md-4">
                            <label for="filter_kode" class="form-label">Kode Warehouse</label>
                            <select class="form-select filter-select" id="filter_kode" multiple="multiple"
                                data-placeholder="Pilih Kode Warehouse...">
                                <!-- Options will be populated dynamically -->
                            </select>
                        </div>
                        <div class="mb-3 col-md-4">
                            <label for="filter_lokasi" class="form-label">Nama Lokasi</label>
                            <select class="form-select filter-select" id="filter_lokasi" multiple="multiple"
                                data-placeholder="Pilih Nama Lokasi...">
                                <!-- Options will be populated dynamically -->
                            </select>
                        </div>
                        <div class="mb-3 col-md-4">
                            <label for="filter_alamat" class="form-label">Alamat</label>
                            <select class="form-select filter-select" id="filter_alamat" multiple="multiple"
                                data-placeholder="Pilih Alamat...">
                                <!-- Options will be populated dynamically -->
                            </select>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#assetsTable').DataTable({
                ...
                ajax: {
                    ...
                    dataSrc: function(json) {
                        populateFilterDropdowns(json.data);
                        return json.data;
                    },
                    data: function(d) {
                        // Add filter parameters to the AJAX request
                        d.kode = $('#filter_kode').val();
                        d.nama_lokasi = $('#filter_lokasi').val();
            ...
            $('#showFilterModal').on('click', function() {
                // Make sure we have the latest data for the filters
            ...
            $('#resetFilters').on('click', function() {
                ...
                $('#filter_kode').val(null).trigger('change');
                $('#filter_lokasi').val(null).trigger('change');
                $('#filter_alamat').val(null).trigger('change');

                // Reload table data to repopulate all filter options
                table.ajax.reload();
            });
            ...
            function populateFilterDropdowns(data) {
                ...
                let selectedKode = $('#filter_kode').val() || [];
                let selectedLokasi = $('#filter_lokasi').val() || [];
                let selectedAlamat = $('#filter_alamat').val() || [];

                // Store unique values
                let kodeValues = new Set();
                let lokasiValues = new Set();
                let alamatValues = new Set();

                // Extract unique values from data
                data.forEach(function(item) {
                    // Add all values regardless of selection status
                    kodeValues.add(item.kode);
                    lokasiValues.add(item.nama_lokasi);
                    alamatValues.add(item.alamat);
                });

                // Update dropdowns while preserving selected values
                updateSelectOptions('filter_kode', kodeValues, selectedKode);
                updateSelectOptions('filter_lokasi', lokasiValues, selectedLokasi);
                updateSelectOptions('filter_alamat', alamatValues, selectedAlamat);
            }

            // Helper function to update select options
            function updateSelectOptions(selectId, newOptions, selectedValues) {
                ...
                newOptions.forEach(function(value) {
                    // Check if this value is in the selectedValues array
                    const isSelected = selectedValues.includes(value);
                    // Add option with selected state set appropriately
                    select.append(new Option(value, value, isSelected, isSelected));
                });
            ...
            $('#btnExport').click(function() {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        saveAs(blob, 'Data_Site_' + new Date().toISOString().split('T')[0] +
                            '.xlsx');
